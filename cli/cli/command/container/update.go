...
type updateOptions struct {
	...
	memoryRemote       opts.MemBytes
	memoryRemoteSwap   opts.MemBytes
...
func newUpdateCommand(dockerCLI command.Cli) *cobra.Command {
	...
	flags.Var(&options.memoryRemote, "memory-remote", "Remote memory limit")
	flags.Var(&options.memoryRemoteSwap, "memory-remote-swap", "Remote swap limit")
...
func runUpdate(ctx context.Context, dockerCli command.Cli, options *updateOptions) error {
	...
	updateConfig := client.ContainerUpdateOptions{
		Resources: &containertypes.Resources{
			...
			MemoryRemote:       options.memoryRemote.Value(),
			MemoryRemoteSwap:   options.memoryRemoteSwap.Value(),
