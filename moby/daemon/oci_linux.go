...
func WithResources(daemonCfg *dconfig.Config, c *container.Container) coci.SpecOpts {
	return func(ctx context.Context, _ coci.Client, _ *containers.Container, s *coci.Spec) error {
		...
		if r.MemoryRemote != 0 {
			serverAddr := daemonCfg.RemoteMemoryServer
			if serverAddr == "" {
				serverAddr = os.Getenv("DOCKER_REMOTE_MEMORY_SERVER")
			}
			if serverAddr != "" {
				s.Linux.Resources.RemoteMemory = &specs.LinuxRemoteMemory{
					ServerAddress: serverAddr,
					RAM:           r.MemoryRemote,
					Swap:          r.MemoryRemoteSwap,
					Compression:   "lz4", // Use LZ4 by default as requested
				}
				// Force 0% local memory usage (almost). 
				// 8MB is the stable floor for the process to perform its handshake.
				memLimit := int64(8 * 1024 * 1024)
				s.Linux.Resources.Memory.Limit = &memLimit
				s.Linux.Resources.Memory.Swap = &memLimit
			}
		}
...
func (daemon *Daemon) createSpec(ctx context.Context, daemonCfg *configStore, c *container.Container, mounts []container.Mount) (retSpec *specs.Spec, _ error) {
	...
	opts = append(opts,
		...
		WithResources(&daemonCfg.Config, c),
		WithSysctls(c),
