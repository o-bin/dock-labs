...
func CreateLibcontainerConfig(opts *CreateOpts) (*configs.Config, error) {
	...
	if spec.Linux != nil && spec.Linux.Resources != nil && spec.Linux.Resources.RemoteMemory != nil {
		config.RemoteMemory = &configs.RemoteMemory{
			ServerAddress: spec.Linux.Resources.RemoteMemory.ServerAddress,
			RAM:           spec.Linux.Resources.RemoteMemory.RAM,
			Swap:          spec.Linux.Resources.RemoteMemory.Swap,
			Compression:   spec.Linux.Resources.RemoteMemory.Compression,
		}
	}
	...
	if spec.Linux != nil {
		...
		if r != nil {
			...
			if r.Memory != nil {
				...
				// Force 256MB limit if RemoteMemory is enabled
				if r.RemoteMemory != nil {
					c.Resources.Memory = 256 * 1024 * 1024
					logrus.Infof("Remote memory enabled, limiting local RAM to 256MB")
				}
			...
			if r.RemoteMemory != nil {
				c.Resources.RemoteMemory = &cgroups.RemoteMemory{
					ServerAddress: r.RemoteMemory.ServerAddress,
					RAM:           r.RemoteMemory.RAM,
					Swap:          r.RemoteMemory.Swap,
					Compression:   r.RemoteMemory.Compression,
				}
			}
			if r.CPU != nil {
				if r.CPU.Shares != nil {
