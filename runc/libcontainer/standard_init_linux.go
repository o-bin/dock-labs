...
func (l *linuxStandardInit) Init() error {
	...
	if l.config.Config.RemoteMemory != nil {
		rm := l.config.Config.RemoteMemory
		poolSize := rm.Swap
		if poolSize == 0 {
			poolSize = rm.RAM
		}
		cmd := exec.Command("/proc/self/exe", "remote-memory-client",
			"--address", rm.ServerAddress,
			"--size", strconv.FormatInt(poolSize, 10),
			"--compression", rm.Compression,
		)
		if err := cmd.Start(); err != nil {
			return fmt.Errorf("failed to start remote memory client: %w", err)
		}
		logrus.Infof("Started remote memory client for %s", rm.ServerAddress)
	}
